...
input[type="submit"].ubah {
    padding: 10px 15px;
    background-color: #007BFF; /* Blue color (for Ubah) */
    color: white;
    border: none;
    cursor: pointer;
    text-align: center;
...
input[type="submit"].hapus {
    padding: 10px 15px;
    background-color: #dc3545; /* Red color (for Hapus) */
    color: white;
    border: none;
    cursor: pointer;
    /* text-align: center; */
...
<?php while($row = mysqli_fetch_array($result)): ?> 
    ...
    <td><?= $row['stok'];?></td> 
    <td class="actions">
        <a href="ubah.php?id=<?= $row['id_barang']; ?>"><input type="submit" class="ubah" value="Ubah" /></a>
        <a href="hapus.php?id=<?= $row['id_barang']; ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')"><input type="submit" class="hapus" value="Hapus" /></a>
    </td>
</tr>
<?php endwhile; else: ?> 
<tr> 
